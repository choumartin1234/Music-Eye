...
def label_clip_to_onehot(label, clip_idx, clip_sec=15, frame_ms=40):
...
if __name__ == '__main__':
    ...
    #         out = os.path.join(outpath, str(i))
    ...
    # with open(os.path.join(outpath, 'map'), "w") as f:
    #     for k, v in dic.items():
    #         f.write(str(k) + '\t' + str(v) + '\n')
    mp3s = [outpath+'/'+i for i in os.listdir(outpath) if i[-4:] == '.mp3']
