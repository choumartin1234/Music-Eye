...
def spectrum(data, fs=44100, target_fs=12800, hop_length=128, cqt=True, fmin=27.5, n_bins=87*4+1, bins_per_octave=4*12):
    ...
    if cqt:
        return np.log2(cqt_spectrum(data, target_fs, hop_length, fmin, n_bins, bins_per_octave) + 0.01)
    else:
        return vggish_input.waveform_to_examples(data, target_fs, return_tensor=False)
